...
devb now defaults to Linux 7.2.2 through a verified fail-closed, no-serial
dispatcher. An all-zero reserved raw state sector at NVMe LBA 61439 selects the
7.2.2 default path. Before loading it, U-Boot writes and reads back the exact
vendor-6.1 fallback marker and starts the RK3588 watchdog. The conditional
systemd keeper clears that marker only after the expected kernel, writable NVMe
root, global IPv4 address and SSH listener are present.

If the default kernel hangs before userspace confirmation, the watchdog resets
the board. The preserved marker is then consumed and vendor 6.1 is booted once.
An invalid/unreadable state or a failure to arm and verify the marker also
fails closed to vendor 6.1. Explicit `wdt61`, `core0` and `3core` one-shot
targets remain available.

The original vendor boot scripts and former default-6.1 dispatcher are
preserved at:

```text
/boot/boot-default-6.1.43-rockchip-rk3588.scr
/boot/boot-default-6.1.43-rockchip-rk3588-backup.scr
/boot/boot-dispatcher-default61.cmd
/boot/boot-dispatcher-default61.scr
```

Both vendor scripts retain SHA-256
`11aeff4bd1870a2ff624fe0f30a79ae9bee5baab3b584621a1af6e7d5e3cf989`.
The preserved default-6.1 dispatcher SHA-256 is
`429322f8ba7de70249ddca9abe4e9239eae911456e8b172ca27fe930caf5f369`.
The active default-7.2.2 dispatcher SHA-256 is
`cd2ff53fc0d70367482746a3413bec0b81528e6befe7316321dd3caf3a3da93e`.

The default path was tested by rebooting with an all-zero state, not by arming
a one-shot target. The resulting command line contained
`devb_boot_default=7.2.2 devb_default_pending=1`; the keeper took over the
watchdog and verified that LBA 61439 had returned to all zeros. Systemd had no
failed units, the sensor APIs and database collectors were healthy, public
Cloudflare routes returned HTTP 200, and Panthor/RKNPU/I2C2/Bluetooth were
present. The dispatcher also resets a watchdog left running across a warm
reset, preventing a reset loop.
...
The vendor kernel payloads remain available. `uInitrd` was regenerated by the
Debian `firmware-mediatek` package so vendor 6.1 can also load the MT7921
firmware:

```text
68e64f9d664c4ad21ffe90e4c84d745b0e957d999b1ae9919cf0d602852363ce  /boot/Image
fa764a084ddcdfe3e64f76583b102d06edaf5de242f3110eb05d5fd47d258927  /boot/uInitrd
afefb78e51740537dfb44d26f9f58f5d84be7243332e673d4749c55f17f643bb  /boot/dtb/rockchip/rk3588-orangepi-5-plus.dtb
```

A plain future reboot now selects Linux 7.2.2. The automatic fallback and an
explicit `/usr/local/sbin/devb-boot-once-state wdt61` still select vendor 6.1.
...
/boot/boot-default-7.2.2-rk3588-panthor-rknpu.cmd
/boot/boot-default-7.2.2-rk3588-panthor-rknpu.scr
/boot/boot-dispatcher-default61.cmd
/boot/boot-dispatcher-default61.scr
...
## Default and no-serial one-shot boot

A plain reboot selects the validated three-core Linux 7.2.2 default. To select
an explicit one-shot target instead:

```sh
/usr/local/sbin/devb-boot-once-state core0
sudo reboot
```

The `3core` target is fully validated and selects the combined three-core DTB.
`wdt61` selects vendor 6.1 once, `show` reports the current raw state, and
`disarm` clears it. The state tool validates the NVMe sector geometry before
every write.

The default dispatcher files have these SHA-256 hashes:

```text
e8b4ead48180da100c1e2304064e33b10867ac2cc0f4752d2bc5e02a18c7d75c  boot-dispatcher.cmd
cd2ff53fc0d70367482746a3413bec0b81528e6befe7316321dd3caf3a3da93e  boot-dispatcher.scr
5b197f203ef8aa62ccfbce3310c225daa27f10258cd3e25f5b5f2793595aa4bd  boot-default-7.2.2-rk3588-panthor-rknpu.cmd
7bf767067e39dae770c4971ca6df424ead96177c6f7e09401baa207afd18af41  boot-default-7.2.2-rk3588-panthor-rknpu.scr
```
...
fail-closed recovery path are validated. Linux 7.2.2 is now the default; vendor
6.1 remains an automatic and explicitly selectable recovery target.
